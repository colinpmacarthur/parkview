...
API_KEY = '6bf06a1e20a0e006e9171149081b75d5'
API_SECRET = '575dd148ad7c58ab'
email = None
password = None
AUTH = False
debug = False
...
class Photo(object):
    ...
    def getURL(self, size='Original', urlType='url'):
...
searchkeywords = photos_search(auth=False,  text='cuyahoga valley national park', per_page=500)
cart = []
for cnt in range(0, len(searchkeywords )):
    box = []
    box.append(searchkeywords[cnt].owner.username)
    box.append(searchkeywords[cnt].owner.realname)
    box.append(searchkeywords[cnt].owner.location)
    box.append(searchkeywords[cnt].dateposted)
    box.append(searchkeywords[cnt].datetaken)
    box.append(searchkeywords[cnt].takengranularity)
    box.append(searchkeywords[cnt].title)
    box.append(searchkeywords[cnt].description)
    box.append(searchkeywords[cnt].url)
    box.append(searchkeywords[cnt].license)
    cart.append(box)
    print(cnt, ' was added.')

print(cart.encode('utf_8'))
print('end')
print(len(searchkeywords), "posts were added to the list.")